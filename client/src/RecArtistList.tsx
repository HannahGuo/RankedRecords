...
export default function RecArtistList() {
	const artistListSelector = useSelector(
		(state: any) => state.artistList.aList,
	)
		...
		if (artistListSelector.length > numArtists) {
			...
			Object.keys(artistRecs).forEach((recID: any) => {
				...
				if (artistMap.includes(newArtist.id)) {
					let newArr = artistRecs[recID].filter(
						(val: any) => val.id !== newArtist.id,
					)
			...
			spotifyApi
				.getArtistRelatedArtists(newArtist.id)
				.then((data) => {
					let artistListIDs = artistListSelector.map(
						(val: ArtistObj) => val.id,
					)
					let artistRecIds = Object.values(updatedArtistRecs).flatMap(
						(val: any[]) =>
					...
					let recs = data.body.artists
						.filter((val: SpotifyArtistObj) => {
							return (
								!artistListIDs.includes(val.id) &&
								!artistRecIds.includes(val.id)
							)
						})
						.slice(0, 5)
	...
	return (
		<div id="recArtistListDiv" className="artistListDiv">
			{artistListSelector.length === 0 ? (
				<div id="noArtistDiv">
					<em>Add artists to see recommendations!</em>
				</div>
			) : null}
