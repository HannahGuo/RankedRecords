...
export default function ArtistList() {
	...
	const loadingStats = useSelector(
		(state: any) => state.songsList.loadingStages,
	)
	...
	return (
		<div id="selectedArtistListDiv" className="artistListDiv">
			{artistListSelector.length === 0 ? (
				<div id="noArtistDiv">
					<em>No artists selected, use search above to add some!</em>
				</div>
			) : null}
							...
							<>
								<span>
									...
									{allSongs.reduce((accumulator, song) => {
										return song.artists.some(
											(artist: SpotifyArtistObj) =>
